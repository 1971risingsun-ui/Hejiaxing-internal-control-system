...
const GlobalPurchasingItems: React.FC<GlobalPurchasingItemsProps> = ({ 
  ...
  const suggestedSupplierNames = useMemo(() => {
    ...
    // 模糊比對：品名/備註 包含 用途關鍵字，或 用途關鍵字 包含 品名/備註
    const matched = suppliers.filter(s => 
      s.productList?.some(p => {
        if (!p.usage) return false;
        // 將用途依半形逗號拆分，逐一進行模糊比對
        return p.usage.split(',').some(k => {
          const kw = k.trim().toLowerCase();
          return kw && (query.includes(kw) || kw.includes(query));
        });
      })
    ).map(s => s.name);
